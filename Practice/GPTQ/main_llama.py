...
사용 예시:
  ...
  python main_llama.py --model meta-llama/Llama-2-7b-hf --bits 4 --rot hadamard --groupsize 128 --compare --dev cuda:0
"""
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("LLaMA Rotated GPTQ  |  김윤준 아이디어")
    print("=" * 60)
    for k, v in vars(args).items():
        print(f"  {k:12s}: {v}")
    print("=" * 60)

    out_rot = run_llama_rot(
    ...
    print(f"  {args.bits}bit RotatedGPTQ ({args.rot:8s}): {out_rot['ppl_quant']:.2f}")
    if args.compare:
